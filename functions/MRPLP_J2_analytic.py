...
class MultiRevolutionPerturbedLambertSolver:
    ...
    class mrplp_J2_analytic_parameters():
        def __init__(self, rr1: np.array, rr2: np.array, tof: float, vv1g: np.array,
                    mu: float, rE: float, J2: float,
                    order: int, tol: float, cont: float, dcontMin: float, scl: float, itermax: int):
    ...
    class output_mrplp_J2_analytic():
        def __init__(self, vv1Sol: np.array, vv2Sol: np.array, rr2DA: np.array, res: list, iter: int, elapsed_time: float, 
                    success: bool):
            self.vv1Sol = vv1Sol
            self.vv2Sol = vv2Sol
            self.rr2DA = rr2DA
    ...
    def defectAnalyticJ2( params: mrplp_J2_analytic_parameters ):
        ...
        # propagate
        xfDA = MultiRevolutionPerturbedLambertSolver.analyticJ2propHill( x0, tof, muSc, params.rE/Lsc, params.J2, cont+scl2*DA(4) )
        ...
        while (residual > errormax) or (exit < 1 and iter < itermax):
            ...
            if iter > itermax:
                if residual > errormax:
                    ...
                    success = False
                    return MultiRevolutionPerturbedLambertSolver.output_mrplp_J2_analytic(vv1Sol, vv2Sol, rr2DA, res, iter, elapsed_time, success)

            # compute the maps and the new dcont
            mapD, dcont, finalState = MultiRevolutionPerturbedLambertSolver.defectAnalyticJ2( params )
            ...
            if cont >= 1:
                ...
                exit = exit + 1
            epsilon.append(cont)
            residual = np.linalg.norm( mapD.eval([0, 0, 0, 0])[0:3] ) # check if you achieved the required target point
            res.append(residual)
            ...
            result = fsolve(MultiRevolutionPerturbedLambertSolver.fsolveFromMap, dvv1Guess, args=(mapD, dcont))
    ...
    def propagatePerturbedKeplerSTM( rr1: np.array, vv1: np.array, tof: float,
        ...
        for indtf in range(len(vecttof)):
            ...
            # propagate
            xfDA = MultiRevolutionPerturbedLambertSolver.analyticJ2propHill( x0DA, vecttof[indtf], muSc, params.rE/Lsc, params.J2, cont+scl2*DA(4) )
    ...
    def propagatePrimerVector(rr1: np.array, vv1: np.array, tof: float,
        ...
        states, vecttof, STM = MultiRevolutionPerturbedLambertSolver.propagatePerturbedKeplerSTM( rr1, vv1, tof, Npoints, params )

        # initial and final conditions for the primer vector
        primerVectorInitialFinalCond = MultiRevolutionPerturbedLambertSolver.primerVectorInitialAndFinalConditions(dvv1, dvv2, STM[-1,:,:])
        ...
        return MultiRevolutionPerturbedLambertSolver.output_primer_vector_propagation(pp, p.flatten(), pd.flatten(), states, vecttof, STM)
