...
class output_mrplp_J2_analytic():
    def __init__(self, vv1Sol: np.array, vv2Sol: np.array, rr2DA: np.array, res: list, iter: int, elapsed_time: float, 
                 success: bool, params: mrplp_J2_analytic_parameters):
        self.vv1Sol = vv1Sol
        self.vv2Sol = vv2Sol
        self.rr2DA = rr2DA
        self.res = res
...
def defectAnalyticJ2( params: mrplp_J2_analytic_parameters ):
    ...
    # propagate
    xfDA = analyticJ2propHill( x0, tof, muSc, params.rE/Lsc, params.J2, cont+scl2*DA(4) )
...
def mrplp_J2_analytic(params: mrplp_J2_analytic_parameters):
    ...
    while (residual > errormax) or (exit < 1 and iter < itermax):
        ...
        mapD, dcont, finalState = defectAnalyticJ2( params )
        ...
        if cont >= 1:
            ...
            exit = exit + 1
        epsilon.append(cont)
        residual = np.linalg.norm( mapD.eval([0, 0, 0, 0])[0:3] )
        res.append(residual)
...
def main():
    ...
    # vv1g = np.array( [-5.33966929176339, -2.54974554812496, 4.35921075804661] )
    ...
    parameters = mrplp_J2_analytic_parameters( rr1, rr2, tof, vv1g, 
                                              order=4, tol=1.0e-6, cont=0.0, dcontMin=0.1, scl=1.0e-3, itermax=200, 
